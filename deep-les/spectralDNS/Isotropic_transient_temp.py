...
if __name__ == "__main__":
    ...
    config.triplyperiodic.add_argument("--dt_ratio", type=int, default=1) # Timestep is set to dt = t_kolmogorov/(N*dt_ratio)
    config.triplyperiodic.add_argument("--ratio_print", type=int, default=32) # Number of times info is printer per kolmogorov time step
    config.triplyperiodic.add_argument("--ratio_save", type=int, default=1) # Number of times fields are saved per kolmogorov time step

    config.triplyperiodic.add_argument("--run", type=int, default=1) # Run ID
    config.triplyperiodic.add_argument("--save_path", type=str, default='../results/') # path to folder
    ...
    config.params.compute_energy = config.params.dt_ratio*config.params.N[0]/config.params.ratio_print   # Compute turb qty 'ratio_print' times per kolmogorov time step
    config.params.compute_spectrum = config.params.dt_ratio*config.params.N[0]/config.params.ratio_print # Compute spectrum 'ratio_print' times per kolmogorov time step
    config.params.write_result = config.params.dt_ratio*config.params.N[0]/config.params.ratio_save      # Save field 'ratio_save' times per kolmogorov time step

    # ------------------------ Initialize turbulence ------------------------- #
    initialize(solver, context)
    # From initialized field compute power spectrum and integral time scale
    ...
    if solver.rank == 0:
        ...
        print('#-------------------------------------------------------------- #')
        print(' Running Homogenous Isotropic Turbulence N={} \n'.format(config.params.N[0]))
        ...
        print('     Integral length scale from forcing {:.5f}'.format(1/config.params.Kf2),flush=True)
        ...
        with open(context.hdf5file.filename[:-3] + '/params.txt', 'w') as f:
            print(config.params, file=f)
