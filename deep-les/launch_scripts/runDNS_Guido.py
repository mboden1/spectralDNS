...
def launchDaint(nCases, les):
    ...
    f.write('#SBATCH --mail-user=mboden@ethz.ch \n')
    ...
    command = 'srun -n 12 python Isotropic_transient.py' + default_params
    command = 'srun -n 12 python Isotropic_transient.py' + default_params
    

    f.write('srun -n 12 python Isotropic_transient.py ${OPTIONS} --Re_lam ${RE[$SLURM_ARRAY_TASK_ID]} --dt_factor ${DT_FACTOR[$SLURM_ARRAY_TASK_ID]}
